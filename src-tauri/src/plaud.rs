...
pub async fn plaud_command<R: Runtime>(app: tauri::AppHandle<R>, action: String, args: Value) -> Result<Value, String> {
  let allowed = ["initialize", "scan", "connect", "refresh", "download", "upload", "snapshot", "play", "stopPlayback", "disconnect", "cancel", "permissions", "exportDocument", "exportRecording", "exportVaultRecording"];
