...
/** Document templates offered by the backend (roadmap TS-07). */
export const DOCUMENT_STYLES = ['notes', 'meeting', 'interview', 'lecture', 'brief', 'prd', 'email'] as const
...
export async function generateDocument(data: { idempotencyKey: string; recordingId: string; versionId: string | null; title: string; style: DocumentStyle; instructions: string; expectedProvider?: string }, onProgress: (stage: string) => void, signal: AbortSignal): Promise<DocumentGeneration> {
